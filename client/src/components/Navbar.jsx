        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          {user ? (
            <div className="relative">
              ...
              {dropdownOpen && (
                <>
                  <div className="fixed inset-0 z-40" onClick={() => { setDropdownOpen(false); setDemoMenuOpen(false); }} />
                  <div className="absolute right-0 mt-2 w-64 glass-dropdown rounded-2xl p-2 z-50 animate-in fade-in slide-in-from-top-2 duration-150 border border-white/10">
                    ...
                    <div className="px-3 py-2 border-b border-white/5 mb-1">
                      ...
                      <div className="mt-1.5 flex flex-wrap gap-1">
                        {(user.favoriteGenres || []).slice(0, 2).map((g, i) => (
                          <span key={i} className="text-[10px] px-1.5 py-0.5 rounded bg-brand-purple/20 text-brand-purple border border-brand-purple/20 font-medium">
                            {g}
                          </span>
                        ))}
                      </div>
                    ...
                    >
                      ...
                      <span>My Taste Profile & Records</span>
                    </button>
                    ...
                    <div className="relative">
                      <button
                        ...
                        className="w-full flex items-center justify-between px-3 py-2 text-xs font-medium text-slate-300 hover:text-white hover:bg-white/10 rounded-xl transition-all text-left"
                      >
                        <div className="flex items-center gap-2.5">
                          <Users className="w-4 h-4 text-brand-cyan" />
                          <span>Switch Persona (Demo)</span>
                        </div>
                        <ChevronDown className={`w-3 h-3 text-slate-400 transition-transform ${demoMenuOpen ? 'rotate-180' : ''}`} />
                      </button>

                      {demoMenuOpen && (
                        <div className="mt-1 p-1 bg-dark-900/95 border border-white/10 rounded-xl space-y-1">
                          {demoUsers.map((dUser) => (
                            <button
                              key={dUser.id}
                              onClick={() => {
                                switchDemoUser(dUser.id);
                                setDropdownOpen(false);
                            ...
                            >
                              <img src={dUser.avatar} alt={dUser.name} className="w-5 h-5 rounded-full object-cover" />
                              <div className="flex-1 min-w-0">
                                <p className="truncate font-medium">{dUser.name}</p>
                                <p className="text-[10px] text-slate-400 truncate">{dUser.favoriteGenres?.[0] || 'Listener'}</p>
                              </div>
                            </button>
                          ))}
                        </div>
            ...
            <button
              onClick={onOpenAuth}
              className="px-4 py-1.5 rounded-full bg-white/10 hover:bg-white/15 border border-white/15 text-white text-xs font-semibold transition-all"
            >
              Sign In / Demo
            </button>
          )}
